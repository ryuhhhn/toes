...
@tool(
    ...
    description=(
        "Find out which one or two questions would most narrow down what the shopper "
        "needs, based on the products currently matching. Returns the attributes worth "
        "asking about with plain-language copy explaining why each matters. You must "
        "phrase the question yourself, in your own words, and only ever alongside "
        "product results — never as a bare interrogation."
    ),
    ...
    parameters=object_schema(
        {
            "limit": {
                "type": "integer",
                "description": "How many attributes to consider. Usually 1 or 2.",
            }
        }
    ),
)
async def probe_attributes(args: dict, ctx: ToolContext) -> ToolResult:
    ...
    limit = min(
        int(args.get("limit") or 1),
        settings.max_probes_per_turn,
        remaining_budget,
    )
    ...
    lines = [
        ...
        "Ask these in your own words, and keep the products on screen."
    ]
