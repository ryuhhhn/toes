...
class PaymentClient:
    ...
    async def authorize(
        self, *, preview_id: str, session_id: str, user_id: str = "demo-user",
        fail: str | None = None,
    ) -> dict:
        return await self._post(
            "/payment/authorize",
            {"preview_id": preview_id, "session_id": session_id, "user_id": user_id},
            {"fail": fail} if fail else None,
        )

    async def confirm(
        self, *, preview_id: str, authorization_id: str, session_id: str,
        fail: str | None = None,
    ) -> dict:
        return await self._post(
            "/payment/confirm",
