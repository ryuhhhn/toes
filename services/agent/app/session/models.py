...
class Session(BaseModel):
    ...
    #: Demo-only failure injection, set by POST /chat/confirm and passed straight to
    #: the payment service's `?fail=`. It can ONLY cause a charge to fail — never to
    #: succeed, and never to skip a gate — so the worst it can do is refuse to take
    #: money. Without it a declined card is unreachable from a conversation: the
    #: mock decline rule needs a total ending in .99, which no real catalogue
    #: obliges, and "the card gets declined" is the beat most worth showing.
...
def new_preview(
    *,
    ...
    """Build the session's copy of a preview.

    `expires_at` is the payment service's own deadline and is preferred whenever it
    sends one. There used to be two clocks — the agent minted 300s from
    PREVIEW_TTL_SECONDS while the ledger enforced 15 minutes — so the shopper was
    shown a deadline nobody enforced. The server is authoritative; `ttl_seconds` is
    only the fallback for a payment service that sends no expiry at all.
    """
    return ActivePreview(
        ...
        expires_at=expires_at or (_now() + timedelta(seconds=ttl_seconds)),
    )
