...
def _parse_expiry(value) -> datetime | None:
    """The payment service's own deadline, or None to fall back to the local TTL.

    Payments enforces this expiry in its ledger, so it is the only one worth showing
    a shopper. Unparseable is not fatal — the local TTL still applies — but it is
    logged, because a preview whose deadline we cannot read is a preview whose
    expiry we are guessing at.
    """
    if not value:
        return None
    try:
        ...
        log.warning("payment service sent an unparseable expires_at: %r", value)
        return None
