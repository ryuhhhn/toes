...
@app.get("/catalog/raw")   # the contract path (docs/CONTRACTS.md §1.1)
@app.get("/catalog")       # legacy alias, kept so older callers do not 404
async def catalog(
...
class PreviewItem(BaseModel):
    # `product_id`, not `id` — this is the key real payments' CartItem uses, and
    # the stub exists to implement the contract, not to be lenient about it.
    product_id: str
    title: str = ""
    quantity: int = 1
...
class AuthorizeRequest(BaseModel):
    preview_id: str
    # Consent IS the authorization: EXPLICIT_CONFIRM means a human pressed the
    # button. No session_id or user_id — real payments takes neither, and the
    # stub accepting them is what let the agent send a body that 422s in prod.
    method: str = "explicit_confirm"
    proof: str | bool = True
...
async def payment_authorize(request: AuthorizeRequest, fail: str | None = Query(None)) -> dict:
    ...
    # UserAuthorizationResult's four fields, exactly.
    return {
        "authorized": True,
        "authorization_id": authorization_id,
        "method": request.method,
...
async def payment_confirm(request: ConfirmRequest, fail: str | None = Query(None)) -> dict:
    ...
    # Real payments returns a Transaction: no items, no subtotal, no tax, and
    # the money field is `amount`, not `total`. The stub used to return a fat
    # receipt, which is precisely why the agent read four fields that do not
    # exist in production. The caller already holds the line items in its own
    # session — that, not this response, is where a receipt is built from.
    transaction = {
    ...
    RECEIPTS[transaction_id] = {
        ...
        "events": [
            {"event_type": "preview_created", "reason": None, "created_at": preview["created_at"]},
            {"event_type": "charge_succeeded", "reason": None, "created_at": _now().isoformat()},
        ],
    }
    return transaction
